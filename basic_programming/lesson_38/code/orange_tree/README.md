...
Release 1 
    ...
    В oranges будет находиться массив апельсинов.
...
Release 3
    Создайте метод isMature(). Метод проверяет достигло ли дерево 6 лет, 
    если оно 6 или старше, то метод возвращает true. Если младше, то false.
    
    Дополните метод .passGrowingSeason() 
    Дерево должно приносить плоды, если оно зрелое 
    (то есть ему не менее шести лет) 
    - скажем, от 100 до 300 апельсинов (class Orange).
    
    Если дерево мертво - оно не приносит апельсинов. 
    (используйте методы, которые уже написали isMature() и isDead()
    вам может понадобится новый массив с длинной, которую вы сгенерируйте
    генератором случайных чисел
    
Release 4 
    Создайте класс Farmer с методом countAverageOrangeDiameter -
        в качестве параметра принимает тип OrangeTree. 
        Возвращает double -  среднее значение диаметра апельсинов с дерева. 
        
        Создайте в main фермера Johanos,пусть он посчитате средний диаметр апельсинов. 
